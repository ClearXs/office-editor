...
const useRequest = (baseUrl: string, docUser: DocUser, cipher: string) => {
   ...
   *
   */
  const resolveResponse = (res: AxiosResponse): Promise<any> => {
  ...
  const resolveIncorrectResponse = (error: AxiosResponse): Promise<any> => {
    const {
      status,
      config: { url },
      statusText,
  ...
  const handleFailedAuthentication = (message: string) => {
    notification.notify({ type: 'error', text: message });
  };
